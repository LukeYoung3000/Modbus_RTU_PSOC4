...
volatile uint8_t _modbus_timer_finished = 0;
static uint8_t _forwarding = 0;
...
static void modbus_reply(int8_t bytes)
{
    ...
    if (_tx_led_enable)
        _tx_led_enable(_tx_led_enable_val);
...
// Might be able to get rid of this function
void modbus_timer_reset(void)
{
    _modbus_timer_reset();
}
...
uint8_t modbus_poll(void)
{
    ...
    if (_modbus_timer_finished)
        goto reset;

    if (!_modbus_read_ready())
        return 0;
...
loop:
    ...
    {
        _modbus_timer_start();
        if (modbus_msg[MODBUS_SLAVE_OFFSET] == _slave_addr)
            msg_type = MSG_INDICATION;
        if (_rx_led_enable)
            _rx_led_enable(_rx_led_enable_val);
    }
...
void modbus_enable_forwarding(uint8_t val)
{
    _forwarding = val;
}
